...
, duration=None, frames_length=None):
    ...
    frames_length = container.streams.video[0].frames  # 203
    duration = container.streams.video[0].duration  # 103936
    ...
    if duration is None:
        ...
        decode_all_video = False
        start_idx, end_idx = get_start_end_idx(  # 0.0 255.0
            frames_length,  # 203
            sampling_rate * num_frames / target_fps * fps,
            clip_idx,  # -1
            num_clips,  # 1
        )
        timebase = duration / frames_length  # 512.0
        video_start_pts = int(start_idx * timebase)  # 0
        video_end_pts = int(end_idx * timebase)  # 130560
...
def decode(
    ...
    assert clip_idx >= -1, "Not valied clip_idx {}".format(clip_idx)  # clip_idx=-1
    try:
        if backend == "pyav":
    ...
    clip_sz = sampling_rate * num_frames / target_fps * fps  # 32*8/30*30
    start_idx, end_idx = get_start_end_idx(
    ...
    if two_token:
        ...
        max_len = frames.shape[0]  # 203

        if rand_fr:  # True
            global_1 = temporal_sampling(frames, 0, max_len - 5, 8)
            global_2 = temporal_sampling(frames, 5, max_len, 16)
            local_samples = []
            local_width = max_len // 8  # 25
