...
class Kinetics(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10, mask_ratio=0.9, get_flow=False, enable_wavelet_viz=True, max_samples=10, save_dir_pth='./output/wavelet_visualizations'):
        """
        ...
        self.enable_wavelet_viz = enable_wavelet_viz and mode == "train"
        if self.enable_wavelet_viz:
            from datasets.wavelet_viz_hook import WaveletVisualizationHook
            print("Constructing wavelet visualization hook...")
            self.wavelet_viz = WaveletVisualizationHook(
    ...
    def set_epoch(self, epoch):
        if self.enable_wavelet_viz:
            self.wavelet_viz.set_epoch(epoch)
    ...
    def _construct_loader(self):
        ...
        with open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                # exit(0)
                assert (
                        len(path_label.split(self.cfg.DATA.PATH_LABEL_SEPARATOR))  # self.cfg.DATA.PATH_LABEL_SEPARATOR ","
                        == 2
                )
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["train", "val"]:
            ...
            if short_cycle_idx in [0, 1]:  # short_cycle_idx=None
                crop_size = int(
                    round(
            ...
            if self.cfg.MULTIGRID.DEFAULT_S > 0:  # self.cfg.MULTIGRID.DEFAULT_S=0
        ...
        for i_try in range(self._num_retries):
            ...
            try:
                video_container = get_video_container(
                    ...
                    self.cfg.DATA_LOADER.ENABLE_MULTI_THREAD_DECODE,  # false
                    self.cfg.DATA.DECODING_BACKEND,  #‘pyav’
                )
            ...
            if self.mode in ["test", "val"] or self.cfg.DATA.NO_RGB_AUG:
                ...
                frames_before = None
                if self.enable_wavelet_viz:
                    frames_before = [x.clone() for x in frames]
                ...
                if self.enable_wavelet_viz and frames_before is not None:
                    self.wavelet_viz.visualize_augmentation(
                        frames_before=frames_before,
                        frames_after=frames,
                        index=index
                    )
                    self.wavelet_viz.visualize_detailed_comparison(
                        frames_before=frames_before,
                        frames_after=frames,
                        index=index
                    )

                # for global_image in frames[:2]:
                for mask_image in frames:
                    _, T_global, H_global, W_global = mask_image.shape
                    mask_function = TubeMaskingGenerator((T_global, H_global // 16, W_global // 16), self.mask_ratio)
                    mask = mask_function()
                    masks.append(mask)
            ...
            if self.get_flow:
                ...
                try:
                    ...
                    print(e)
                    continue
            return frames, masks, label, index, meta_data
