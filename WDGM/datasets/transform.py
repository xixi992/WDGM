...
try:
    from datasets.wt_aug import WaveletRandomMask
    _HAS_WAVELET_RANDOM_MASK = True
except Exception:
    WaveletRandomMask = None
    _HAS_WAVELET_RANDOM_MASK = False
...
def wavelet_feature_augment(frames, wavelet: str = "haar", level: int = 1, sigma: float = 0.1):

    if not _HAS_PYWT:
        return frames

    if frames.ndim != 4:
        raise ValueError(f"wavelet_feature_augment expects frames with shape T x C x H x W, got {frames.shape}")
    ...
    out_np = np.empty_like(frames_np)
    ...
    for ti in range(t):
        for ci in range(c):
            ...
            for (cH, cV, cD) in detail_coeffs:
                scale_H = 1.0 + np.random.normal(0.0, sigma)
                scale_V = 1.0 + np.random.normal(0.0, sigma)
                scale_D = 1.0 + np.random.normal(0.0, sigma)
                new_detail_coeffs.append(
                    (cH * scale_H, cV * scale_V, cD * scale_D)
                )
...
class VideoDataAugmentationDINO(object):
    def __init__(self, global_crops_scale=(0.4, 1.0), local_crops_scale=(0.05, 0.4), local_crops_number=8):
        ...
        if _HAS_WAVELET_RANDOM_MASK:
            self.wavelet_mask = WaveletRandomMask(drop_rate=0.3, separate_channels=False)
        else:
    ...
    def wt_aug(self, frames, p: float = 0.5):

        if np.random.uniform() < p:
            if self.wavelet_mask is not None:
                frames = self.wavelet_mask(frames)
            else:
                frames = wavelet_feature_augment(frames)
        return frames
