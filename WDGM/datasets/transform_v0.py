...
class WaveletTransform:
    def __init__(self, wavelet='db1', enhance_coeff=1.5):
        self.wavelet = wavelet
        self.enhance_coeff = enhance_coeff
        self.dwt = DWTForward(wave=self.wavelet, J=1)
        self.idwt = DWTInverse(wave=self.wavelet)

    def _validate_input_dims(self, image):
    ...
    def _safe_squeeze(self, tensor, target_dims=3):
        current_dims = tensor.dim()
        if current_dims <= target_dims:
            return tensor

        dims_to_squeeze = []
        for dim in range(current_dims):
            if tensor.size(dim) == 1 and current_dims - len(dims_to_squeeze) > target_dims:
    ...
    def _enhance_coefficients(self, coeffs):
        """
        Enhance wavelet coefficients.
        pytorch-wavelets returns: (yl, [yh])
        """
        yl, yh = coeffs  # yl: low freq, yh: list of high freq

        # Enhance high-frequency coefficients
        enhanced_yh = []
        for yh_level in yh:
            if isinstance(yh_level, (list, tuple)):
                enhanced_level = [coeff * self.enhance_coeff for coeff in yh_level]
            else:
                enhanced_level = yh_level * self.enhance_coeff
            enhanced_yh.append(enhanced_level)

        return (yl, enhanced_yh)

    def __call__(self, image):
        """Apply wavelet transform, handling video data by processing frame-by-frame."""
        original_shape = image.shape
        original_dim = image.dim()

        # Validate input
        image = self._validate_input_dims(image)

        # Handle video data: collapse temporal/batch dimensions
        if image.dim() > 4:
            # Reshape to [N*T*..., C, H, W] for processing
            *batch_dims, C, H, W = image.shape
            num_frames = 1
            for d in batch_dims:
                num_frames *= d
            image = image.reshape(num_frames, C, H, W)
            is_video = True
        else:
            is_video = False

        # Process based on channels
        if image.size(1) == 3:  # RGB: [N, 3, H, W]
            coeffs = self.dwt(image)
            coeffs = self._enhance_coefficients(coeffs)
            enhanced = self.idwt(coeffs)
            result = enhanced
        elif image.size(1) == 1:  # Grayscale: [N, 1, H, W]
            coeffs = self.dwt(image)
            coeffs = self._enhance_coefficients(coeffs)
            enhanced = self.idwt(coeffs)
            result = enhanced
            ...
            for i in range(image.size(1)):
                single_channel = image[:, i:i + 1, :, :]
                coeffs = self.dwt(single_channel)
                coeffs = self._enhance_coefficients(coeffs)
                enhanced = self.idwt(coeffs)
        ...
        elif original_dim == 3 and result.dim() == 4:
...
class AdaptiveDimensionDINO:
    def __init__(self, base_size=224, size_ratio=0.5):
        self.base_size = base_size
        self.local_size = int(base_size * size_ratio)

    def _get_adaptive_size(self, image, mode='global'):
        h, w = image.shape[-2:]

        if mode == 'global':
            target_size = self.base_size
        else:  # local
            target_size = self.local_size
        if target_size % 2 != 0:
            target_size = target_size - 1

        return target_size

    def transform_with_adaptive_size(self, image, mode='global'):
        target_size = self._get_adaptive_size(image, mode)
        image_resized = random_resized_crop(image, size=target_size)
...
class VideoDataAugmentationDINO(object):
    def __init__(self, global_crops_scale=(0.4, 1.0), local_crops_scale=(0.05, 0.4), local_crops_number=8):
        ...
        self.global_wavelet = WaveletTransform(wavelet='db1', enhance_coeff=0.8)  # Low freq enhancement for global
        self.local_wavelet = WaveletTransform(wavelet='db1', enhance_coeff=1.5)  # High freq enhancement for local
    ...
    # first global crop with low frequency enhancement
    def global_transform1(self, frames):
        ...
        # Apply wavelet transform with low frequency enhancement
        frames = self.global_wavelet(frames)
        frames = self.gaussian_blur(frames)
        frames = self.normalize(frames)
        return frames

    # second global crop with low frequency enhancement
    ...
    # transformation for local crops with high frequency enhancement
    def local_transform(self, frames):
        ...
        frames = self.flip_and_color_jitter(frames)
        # Apply wavelet transform with high frequency enhancement
        frames = self.local_wavelet(frames)
