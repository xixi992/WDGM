...
def eval_finetune(args):
    ...
    if config.DATA.USE_FLOW or config.MODEL.TWO_TOKEN:
        ...
        if args.arch == "vit_base":  # Here
            ...
            model_embed_dim = model.embed_dim  # 768
        elif args.arch == "swin":
    ...
    if not args.scratch and args.pretrained_weights:  # Here
    ...
    linear_classifier = LinearClassifier(model_embed_dim * (args.n_last_blocks + int(args.avgpool_patchtokens)),
                                         num_labels=args.num_labels)  # 768 2
    linear_classifier = linear_classifier.cuda()
    linear_classifier = nn.parallel.DistributedDataParallel(linear_classifier, device_ids=[args.gpu],find_unused_parameters=True)

    if args.test:
        checkpoint = torch.load(args.pretrained_model_weights, map_location='cpu')
        print(f"Loading checkpoint from: {args.pretrained_model_weights}")

        if "backbone_state_dict" in checkpoint:
            msg = model.load_state_dict(checkpoint["backbone_state_dict"], strict=False)
            print(f"Loaded backbone with msg: {msg}")
        else:
            print("WARNING: No backbone_state_dict in checkpoint!")

        if "state_dict" in checkpoint:
            msg = linear_classifier.load_state_dict(checkpoint["state_dict"], strict=False)
            print(f"Loaded linear_classifier with msg: {msg}")
        else:
            ...
            if f1 > best_f1:
                ...
                if utils.is_main_process():
                    ...
                    save_dict = {
                        ...
                        "backbone_state_dict": model.state_dict(),  #add backbone
                        "optimizer": optimizer.state_dict(),
                        "scheduler": scheduler.state_dict(),
                        "best_f1": best_f1,
                    }
                    torch.save(save_dict, os.path.join(args.output_dir, "checkpoint.pth.tar"))
                    print(f"? Saved checkpoint with backbone_state_dict to {args.output_dir}/checkpoint.pth.tar")
        print(f'Final Max F1 score so far: {best_f1 * 100:.1f}%')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='vit_base', type=str,
                        ...
                        help='Architecture (supportepochs only ViT atm).')
    ...
    parser.add_argument('--data_path', default='data/downstream/PolypDiag', type=str)
    parser.add_argument('--num_workers', default=10, type=int, help='Number of data loading workers per GPU.')
    parser.add_argument('--val_freq', default=1, type=int, help="Epoch frequency for validation.")
    parser.add_argument('--output_dir', default=".checkpoints/debug_classification", help='Path to save logs and checkpoints')
    ...
    parser.add_argument("--opts", help="See utils/defaults.py for all options", default=['DATA.PATH_TO_DATA_DIR', 'data/downstream/PolypDiag/splits', 'DATA.PATH_PREFIX', 'data/downstream/PolypDiag/videos', 'DATA.USE_FLOW', 'False'], nargs=argparse.REMAINDER)
