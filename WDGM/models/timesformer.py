...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(
                ...
                out_channels=self.encoder_stride ** 2 * 3, kernel_size=1),  # 16*16*3
    ...
    def forward_features(self, x, mask, get_attn=False, get_all=False):
        B = x.shape[0]
        th = x.shape[-1] // 16
        x_origin = x
        ...
        if mask is not None:
            BT, _, C = x.shape
            x = rearrange(x, '(b t) n m -> (b t) m n', b=B, t=T)
            x = x.view(BT, C, th, th)
        ...
        if get_attn:  # false
            for i, blk in enumerate(self.blocks):
                if i < len(self.blocks) - 1:
                    ...
                    x = blk(x, B, T, W)
                    attn_last = blk(t, B, T, W, return_attn=True)
        else:
            # Attention blocks
        ...
        if mask is not None:
            ...
            mask = mask.repeat_interleave(self.patch_size, 1).repeat_interleave(self.patch_size, 2).unsqueeze(
                1).contiguous()
            x_origin = x_origin.permute(0, 2, 1, 3, 4)
            x_origin = rearrange(x_origin, 'b t c h w -> (b t) c h w')
            loss_recon = F.l1_loss(x_origin, x_rec, reduction='none')
            loss = (loss_recon * mask).sum() / (mask.sum() + 1e-5) / 3
            return loss, x_rec
        else:
            if get_attn:
                return x[:, 0], attn_last
            else:
                return x[:, 0]

    def forward(self, x, mask=None, get_attn=False ,use_head=False):
        x = self.forward_features(x, mask, get_attn)
        if use_head:
            x = self.head(x)
        return x
