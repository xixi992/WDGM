...
class DINOHead(nn.Module):
    ...
    def forward(self, x):
        x = self.mlp(x)  # [30,256]
        x = nn.functional.normalize(x, dim=-1, p=2)   # [30,256]
        x = self.last_layer(x)  # [30,65536]
        return x
...
class MultiDINOHead(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, nlayers=3, hidden_dim=2048,
        ...
        if nlayers == 1:
            self.mlp = nn.Linear(in_dim, bottleneck_dim)
            self.aux_mlp = nn.Linear(in_dim, bottleneck_dim)
        else:
            layers = [nn.Linear(in_dim, hidden_dim)]
            if use_bn:
                layers.append(nn.BatchNorm1d(hidden_dim))
            ...
            aux_layers = [nn.Linear(in_dim, hidden_dim)]
            if use_bn:
                aux_layers.append(nn.BatchNorm1d(hidden_dim))
            aux_layers.append(nn.GELU())
            for _ in range(nlayers - 2):
                aux_layers.append(nn.Linear(hidden_dim, hidden_dim))
                if use_bn:
                    aux_layers.append(nn.BatchNorm1d(hidden_dim))
                aux_layers.append(nn.GELU())
            aux_layers.append(nn.Linear(hidden_dim, bottleneck_dim))
        ...
        self.aux_last_layer = nn.utils.weight_norm(nn.Linear(bottleneck_dim, out_dim, bias=False))
    ...
    def forward(self, x):
        ...
        aux_x = nn.functional.normalize(aux_x, dim=-1, p=2)
